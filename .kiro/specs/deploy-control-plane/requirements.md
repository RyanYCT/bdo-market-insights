...
equivalent. Four capabilities are exposed — **config**, **bootstrap**,
...
`bdo_common`. This feature adds no new AWS infrastructure.
...
1. THE Wizard SHALL provide CLI_Mode as a non-interactive front-end that accepts one subcommand per capability (`config`, `bootstrap`, `deploy`, `release`) and the flags `--json`, `--yes`, and `--dry-run`, and SHALL NOT prompt for any input.
...
1. WHEN `config show` is invoked for a stage, THE ConfigStore SHALL render a merged read of `samconfig.toml` and SSM for that stage and SHALL NOT mutate any source.
...
- Runtime feature flags, deferred to a follow-up spec. When they are introduced,
  the sanctioned store is DynamoDB — read in Lambdas via Powertools through the
  existing free DynamoDB Gateway endpoint — rather than AWS AppConfig, which
  would require a paid PrivateLink interface endpoint under the no-NAT
  constraint (ADR-0006).
