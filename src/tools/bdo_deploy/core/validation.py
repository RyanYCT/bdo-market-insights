...
- A ``config set`` key that is secret-shaped is refused unless the target stage
  already carries it (``samconfig_parameter_names``), which is read from the same
  cached document as the stage set — planning stays pure and adds no I/O
  (Requirement 3.8).
...
_DEPLOY_PARAMETERS_PATH: Final = ("deploy", "parameters")
"""Where a stage's SAM deploy settings live: ``[<stage>.deploy.parameters]``."""

_PARAMETER_OVERRIDES_KEY: Final = "parameter_overrides"
"""The entry holding the stage's CloudFormation parameter set."""


@cache
def _samconfig_document(path: Path) -> dict[str, Any]:
    """Read and parse ``samconfig.toml`` **once** per path, and cache it.

    This is *the* cached read the module docstring refers to: every rule here —
    the stage set, the SSM stage segment, the per-stage parameter names — is
    derived from this one parse, so a ``Command`` construction plus the planning
    that follows it touches the file a single time. Keyed by path so a test can
    point the layer at a fixture file without sharing the real file's entry
    (``_samconfig_document.cache_clear()`` drops both).
...
def samconfig_stages(path: Path | None = None) -> frozenset[str]:
    ...
    ``path`` defaults to ``SAMCONFIG_PATH`` *at call time* rather than binding it
    as a default argument value, so a test that repoints the module constant
    repoints the whole validation layer with it.
    """
    document = _samconfig_document(path or SAMCONFIG_PATH)
    return frozenset(
        name for name, table in document.items() if isinstance(table, dict) and name != "default"
    )


def samconfig_parameter_names(stage: str, path: Path | None = None) -> frozenset[str]:
    """Return the keys ``stage`` already carries in ``samconfig.toml``.

    The union of the ``[<stage>.deploy.parameters]`` entries and the
    CloudFormation parameters lifted out of that table's ``parameter_overrides``
    — the same flattening a merged ``config show`` read performs (ADR-0036) — so
    ``BdoRegions``, which exists only inside the overrides string, counts as
    present. Both spellings SAM accepts for that entry are handled: one
    space-delimited ``Key=Value`` string, or a TOML array of them.

    Per stage, deliberately: the two stages' parameter sets can differ, and a key
    committed for ``dev`` says nothing about ``prod``.

    An absent or oddly-shaped table yields an empty set rather than an error.
    The one caller is Requirement 3.8's allowlist, where "no names are known to
    be present" means *refuse the secret-shaped key* — the safe reading. Shape
    complaints belong to the executor that has to edit the table, not to a
    predicate asked whether a name is already there.
    """
    node: Any = _samconfig_document(path or SAMCONFIG_PATH).get(stage)
    for segment in _DEPLOY_PARAMETERS_PATH:
        if not isinstance(node, dict):
            return frozenset()
        node = node.get(segment)
    if not isinstance(node, dict):
        return frozenset()
    return frozenset(str(key) for key in node) | _override_names(
        node.get(_PARAMETER_OVERRIDES_KEY)
    )


def _override_names(overrides: object) -> frozenset[str]:
    """The parameter names inside a ``parameter_overrides`` entry.

    Accepts the space-delimited string and the TOML-array forms; an entry that is
    neither contributes nothing.
    """
    if isinstance(overrides, str):
        entries = overrides.split()
    elif isinstance(overrides, list):
        entries = [entry for entry in overrides if isinstance(entry, str)]
    else:
        return frozenset()
    return frozenset(
        key
        for entry in entries
        for key, separator, _ in [entry.partition("=")]
